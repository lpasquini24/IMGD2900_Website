...
PS.init = function( system, options ) {
    ...
    var umbrellaCatUpLImage, umbrellaCatDownLImage, umbrellaCatUpDImage, umbrellaCatDownDImage;
    PS.imageLoad( "Images/UmbrellaCat-Up-Light.png", function (data) {
        umbrellaCatUpLImage = data; // save image ID
        // create sprites from images
        for(let i = 0; i < 4; i++)
        {
            let umbrellaCatUpSprite = PS.spriteImage(umbrellaCatUpLImage);
            // place some images!
    ...
    PS.imageLoad( "Images/UmbrellaCat-Down-Light.png", function (data) {
        umbrellaCatDownLImage = data; // save image ID
        // create sprites from images
        for(let i = 0; i < 4; i++)
        {
            let umbrellaCatDownSprite = PS.spriteImage( umbrellaCatDownLImage );
    ...
    PS.imageLoad( "Images/UmbrellaCat-Up-Dark.png", function (data) {
        // create sprites from images
        for(let i = 0; i < 4; i+=2)
        {
            let umbrellaCatUpSprite = PS.spriteImage(data);

            PS.spriteShow(umbrellaCatUpSprite, false);
            PS.spriteAxis(umbrellaCatUpSprite, 0, 8);
            PS.spriteMove(umbrellaCatUpSprite, 7 * i, height);

            catUpSprites[i] = (umbrellaCatUpSprite);
        }

    } );


    PS.imageLoad( "Images/UmbrellaCat-Down-Dark.png", function (data) {
        // create sprites from images
        for(let i = 0; i < 4; i+=2)
        {
            let umbrellaCatDownSprite = PS.spriteImage( data );
            PS.spriteShow(umbrellaCatDownSprite, true);
            PS.spriteAxis(umbrellaCatDownSprite, 0, 8);
            PS.spriteMove(umbrellaCatDownSprite, 7 * i, height);

            catDownSprites[i] = umbrellaCatDownSprite;
        }
    } );
...
PS.getBackgroundColor = function(x){
    let color = (Math.floor(x / 7) % 2) === 0 ? 0x70d7ed : 0x80e5fa;
    return color;
}

PS.drawBackground = function(){
    PS.border(PS.ALL, PS.ALL, 0);
    for(let x = 0; x < width; x++){
        PS.color(x, PS.ALL, PS.getBackgroundColor(x));
    }
};
...
PS.drawEmpty = function(x, y) {
    if(PS.isInBounds(x, y)) {
        PS.color(x, y, PS.getBackgroundColor(x));
    }
    PS.drawCats();
}
